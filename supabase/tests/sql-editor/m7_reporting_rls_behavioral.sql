...
-- This is a hand-converted copy of supabase/tests/m7_reporting_rls_behavioral.sql
...
-- fixture insert, role/RLS impersonation step, assertion, the wrapping BEGIN,
-- and the final ROLLBACK are preserved exactly. Each `assert` raises an
-- exception that aborts the run, and the whole run is ROLLED BACK — it mutates
-- nothing.
--
